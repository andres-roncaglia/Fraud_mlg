...
Además, se propucieron ciertas interacciones: 
...
:Test de comprabación de la función de enlace y bondad de ajuste
...
$$
logit(\pi_i) = \hat\beta_0 + \hat\beta_I\cdot I_i + \hat\beta_{H1}\cdot H_{1i} + \hat\beta_{H2}\cdot H_{2i} + \hat\beta_{H3}\cdot H_{3i} + \hat\beta_{H4}\cdot H_{4i} + \hat\beta_{H5}\cdot H_{5i} + \hat\beta_{Ph}\cdot Ph_i +$$
$$+ \hat\beta_{Pm}\cdot Pm_i + \hat\beta_C\cdot C_i + \hat\beta_L\cdot L_i + \hat\beta_{D1}\cdot D_{1i} + \hat\beta_{D2}\cdot D_{2i} + \hat\beta_{D3}\cdot D_{3i} + \hat\beta_{D4}\cdot D_{4i} + \hat\beta_E\cdot E_i + \hat\beta_{IPm}\cdot I_i\cdot Pm_i + \hat\beta_{CPm}\cdot C_i\cdot Pm_i$$
...
tabla1 <- data.frame(Beta = c("$\\hat\\beta_0$","$\\hat\\beta_{I}$", "$\\hat\\beta_{H1}$", "$\\hat\\beta_{H2}$", "$\\hat\\beta_{H3}$", "$\\hat\\beta_{H4}$", "$\\hat\\beta_{H5}$", "$\\hat\\beta_{Ph}$"),
                     Efecto = c("Intercepto", "Ingreso", "Estado residencial:BB",
                      "Estado residencial:BC", "Estado residencial:BD",
                      "Estado residencial:BE","Estado residencial:BF",
                      "Teléfono fijo:Válido"),
           Coeficiente = modelo_logit$coefficients[1:8],row.names = NULL,
           LI = modelo_logit$coefficients[1:8] - x$coefficients[1:8,2]*1.96, 
           LS = modelo_logit$coefficients[1:8] + x$coefficients[1:8,2]*1.96
           )

tabla2 <- data.frame(Beta = c("$\\hat\\beta_{Pm}$", "$\\hat\\beta_{C}$", "$\\hat\\beta_{L}$", "$\\hat\\beta_{D1}$", "$\\hat\\beta_{D2}$", "$\\hat\\beta_{D3}$", "$\\hat\\beta_{D4}$", "$\\hat\\beta_{E}$"),
                     Efecto = c("Teléfono móvil:Válido", "Otra tarjeta:Si", "Límite propuesto", "Sistema operativo:MacOS", "Sistema operativo:Otro", "Sistema operativo:Windows", "Sistema operativo:OSx11", "N° de emails distintos (8 sem)"),
           Coeficiente = modelo_logit$coefficients[9:16],row.names = NULL,
           LI = modelo_logit$coefficients[9:16] - x$coefficients[9:16,2]*1.96, 
           LS = modelo_logit$coefficients[9:16] + x$coefficients[9:16,2]*1.96
           )
...
```{r capacidad predictiva modelo con interaccion, eval=FALSE}
