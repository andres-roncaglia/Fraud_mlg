...
(data |> 
  ...
  geom_bar(color = "black", fill = c("#4A5899", "#F2545B")) +
  xlab(label = "Fraude") +
  ...
  geom_bar(color = "black", fill = "#4A5899") +
...
(data |> 
   ...
   ggplot() +
   aes(x = income, y = (after_stat(count))/sum(after_stat(count)))+
   geom_bar(color = "black", fill = "#F2545B") +
   xlab(label = "Ingreso anual") +
   ylab(label = "Proporción") +
   scale_y_continuous(limits = c(0,0.6)) +
   scale_x_continuous(breaks = seq(0.1,0.9,0.1)) +
...
(data |> 
  ...
  ylab(label = "Proporción") +
  scale_y_continuous(limits = c(0,0.65))+
  ggtitle("Fraude")) + plot_layout(axes = "collect") + # desde acá
  (data |> 
  filter(fraud_bool == 0) |> 
  ggplot() +
...
data.frame(
  ...
  est_rv = c(0.8010, 1.9706, 0.0015),
  gl_rv = c(1, 1, 1),
  p_value_rv = c(0.3708, 0.1604, 0.9682),
  est_hl = c(2.7342, 5.7644, 2.6860),
  gl_hl = c(8, 8, 8),
  p_value_hl = c(0.9499, 0.6736, 0.9525)
) |> 
  `colnames<-`(c("Enlace", "Est. RV", "GL RV", "Valor-p RV", "Est. HL", "GL HL", "Valor-p HL")) |>
  kable() |> 
  kable_styling() |> 
  row_spec(0, background = "#F2545B", color = "grey90") |> 
  row_spec(1:3, background = "#191919", color = "grey90") |> 
...
kable(tabla_ro, digits = 4) |> 
  kable_styling() |> 
  row_spec(0, background = "#F2545B", color = "grey90") |> 
  row_spec(1:4, background = "#191919", color = "grey90") |> 
  column_spec(1, background = "#191919")
